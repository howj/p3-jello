...
public class ParallelSearcher2<M extends Move<M>, B extends Board<M, B>> extends
    ...
    public M getBestMove(B board, int myTime, int opTime) {
    	...
    	return ((BestMove<M>)POOL.invoke(new SearchTask<M, B>(moves, 0, moves.size(), this.evaluator, board, ply, cutoff))).move;
    }
    
    public M getBestMove(B board, int myTime, int opTime, int cores) {
    	final ForkJoinPool POOL2 = new ForkJoinPool(cores);
    	List<M> moves = board.generateMoves();
    	return ((BestMove<M>)POOL2.invoke(new SearchTask<M, B>(moves, 0, moves.size(), this.evaluator, board, ply, cutoff))).move;
    }
